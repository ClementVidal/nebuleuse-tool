...
export async function updateTemplate(templateId: string, changes: Partial<Omit<NodeTemplate, 'id' | 'projectId'>>) {
...
/** Returns the map opened by a node, creating it on first access. */
export async function ensureChildMap(nodeId: string): Promise<string> {
  return db.transaction('rw', [db.nodes, db.maps], async () => {
    const node = await db.nodes.get(nodeId)
    if (!node) throw new Error(`Node ${nodeId} not found`)
    if (node.childMapId) return node.childMapId
...
export async function updateNode(nodeId: string, changes: Partial<Omit<IdeaNode, 'id' | 'projectId' | 'mapId'>>) {
  await db.nodes.update(nodeId, changes)
}
...
export async function updateNodeValue(nodeId: string, fieldId: string, value: FieldValue) {
  await db.nodes.update(nodeId, { [`values.${fieldId}`]: value })
}
...
export async function updateEdge(edgeId: string, changes: Partial<Omit<IdeaEdge, 'id' | 'projectId' | 'mapId'>>) {
  await db.edges.update(edgeId, changes)
}
